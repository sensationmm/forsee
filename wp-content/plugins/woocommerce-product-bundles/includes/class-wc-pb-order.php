...
class WC_PB_Order {
	 ...
	 * @return array
	 */
	public static function get_bundle_parent( $item, $order ) {
	 ...
	 * @return array
	 */
	public static function get_bundle_children( $item, $order ) {
		...
		if ( ! isset( $item[ 'bundled_by' ] ) && isset( $item[ 'stamp' ] ) ) {
			...
			if ( ! empty( $children ) ) {
				...
				foreach ( $children as $child ) {
					...
					if ( WC_PB()->compatibility->is_item_subscription( $order, $child ) ) {
						if ( $child[ 'subscription_recurring_amount' ] > 0 ) {
