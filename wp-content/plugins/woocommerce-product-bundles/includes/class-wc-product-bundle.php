...
class WC_Product_Bundle extends WC_Product {
	 ...
	 * @var array
	 */
	public $bundled_quantities_index = array( 'min' => array(), 'max' => array() );
	...
	public function sync_bundle() {
		...
		if ( $this->is_priced_per_product() ) {
			...
			if ( $this->base_sale_price != $this->base_regular_price && $this->base_sale_price == $this->base_price ) {
				$this->on_sale = true;
			}
	...
	private function load_price_data() {
		...
		$this->bundle_price_data[ 'per_product_pricing' ]      = $this->is_priced_per_product() ? 'yes' : 'no';
		$this->bundle_price_data[ 'bundle_is_composited' ]     = apply_filters( 'woocommerce_bundle_is_composited', false, $this ) ? 'yes' : 'no';
		$this->bundle_price_data[ 'show_free_string' ]         = ( $this->is_priced_per_product() ? apply_filters( 'woocommerce_bundle_show_free_string', false, $this ) : true ) ? 'yes' : 'no';
		$this->bundle_price_data[ 'prices' ]                   = array();
		$this->bundle_price_data[ 'regular_prices' ]           = array();

		$this->bundle_price_data[ 'recurring_prices' ]         = array();
		$this->bundle_price_data[ 'regular_recurring_prices' ] = array();
		$this->bundle_price_data[ 'recurring_html' ]           = array();
		$this->bundle_price_data[ 'recurring_keys' ]           = array();

		if ( $this->is_priced_per_product() ) {
			$this->bundle_price_data[ 'base_price' ]         = WC_PB_Helpers::get_product_display_price( $this, $this->get_base_price() );
			$this->bundle_price_data[ 'base_regular_price' ] = WC_PB_Helpers::get_product_display_price( $this, $this->get_base_regular_price() );
		} else {
			$this->bundle_price_data[ 'base_price' ]         = $this->get_price() === '' ? '' : WC_PB_Helpers::get_product_display_price( $this, $this->get_price() );
			$this->bundle_price_data[ 'base_regular_price' ] = $this->get_regular_price() === '' ? '' : WC_PB_Helpers::get_product_display_price( $this, $this->get_regular_price() );
		}

		$this->bundle_price_data[ 'total' ]         = 0.0;
		$this->bundle_price_data[ 'regular_total' ] = 0.0;
		...
		foreach ( $this->bundled_items as $bundled_item ) {
			...
			$this->bundle_price_data[ 'recurring_prices' ][ $bundled_item->item_id ]         = '';
			$this->bundle_price_data[ 'regular_recurring_prices' ][ $bundled_item->item_id ] = '';

			// Store sub recurring key for summation (variable sub keys are stored in variations data).
			$this->bundle_price_data[ 'recurring_html' ][ $bundled_item->item_id ]           = '';
			$this->bundle_price_data[ 'recurring_keys' ][ $bundled_item->item_id ]           = '';

			if ( $bundled_item->is_sub() && ! $bundled_item->is_variable_sub() ) {
				$this->bundle_price_data[ 'recurring_prices' ][ $bundled_item->item_id ]         = $bundled_item->get_bundled_item_recurring_price( 'min', true );
				$this->bundle_price_data[ 'regular_recurring_prices' ][ $bundled_item->item_id ] = $bundled_item->get_bundled_item_regular_recurring_price( 'min', true );
				$this->bundle_price_data[ 'recurring_keys' ][ $bundled_item->item_id ]           = str_replace( '_synced', '', WC_Subscriptions_Cart::get_recurring_cart_key( array( 'data' => $bundled_item->product ), ' ' ) );
				$this->bundle_price_data[ 'recurring_html' ][ $bundled_item->item_id ]           = WC_PB_Helpers::get_recurring_price_html_component( $bundled_item->product );
			}
		}

		if ( $this->is_priced_per_product() && $this->contains_sub ) {
			if ( $this->min_bundle_regular_price != 0 ) {
				$this->bundle_price_data[ 'price_string' ] = sprintf( _x( '%1$s<span class="bundled_subscriptions_price_html" style="display:none"> now,</br>then %2$s</span>', 'subscription price html suffix', 'woocommerce-product-bundles' ), '%s', '%r' );
			} else {
				$this->bundle_price_data[ 'price_string' ] = '<span class="bundled_subscriptions_price_html">%r</span>';
			}
		} else {
	...
	public function get_bundle_price( $min_or_max = 'min', $display = false ) {
		...
		if ( $this->is_priced_per_product() ) {
			...
			if ( $price && $display ) {
				...
				$display_price = WC_PB_Helpers::get_product_display_price( $this, $this->get_base_price() );
