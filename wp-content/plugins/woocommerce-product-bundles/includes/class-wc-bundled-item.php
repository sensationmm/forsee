...
class WC_Bundled_Item {
	...
	/**
	 * Bundled item prices (after discount, no filters applied).
	 * @var double
	 */
	public $min_price;
	public $max_price;
	...
	private $min_price_product;
	private $max_price_product;
	private $min_regular_price_product;
	private $max_regular_price_product;
	 ...
	 * @return mixed
	 */
	private function get_bundled_product_regular_price( $product = false ) {
	...
	public function get_bundled_item_price( $min_or_max = 'min', $display = false ) {
		...
		$price   = apply_filters( 'woocommerce_get_price', $price, $product );
	...
	public function get_bundled_item_recurring_price( $min_or_max = 'min', $display = false ) {
		...
		$prop    = $min_or_max . '_price_product';
		$product = ! empty( $this->$prop ) ? $this->$prop : $this->product;
		$price   = $min_or_max === 'min' ? $this->min_recurring_price : $this->max_recurring_price;
	...
	public function get_bundled_item_regular_recurring_price( $min_or_max = 'min', $display = false ) {
		...
		$prop    = $min_or_max . '_regular_price_product';
		$product = ! empty( $this->$prop ) ? $this->$prop : $this->product;
		$price   = $min_or_max === 'min' ? $this->min_regular_recurring_price : $this->max_regular_recurring_price;
			...
			if ( 0 == WC_Subscriptions_Product::get_trial_length( $product ) ) {
				...
				if ( WC_Subscriptions_Synchroniser::is_product_prorated( $product ) ) {
					...
					if ( WC_Subscriptions_Synchroniser::is_today( $next_payment_date ) ) {
						return $sign_up_fee + $recurring_price;
					}
					...
					$price = $sign_up_fee + $recurring_price;
				}
			}
		}
