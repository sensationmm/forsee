...
class HubSpot_Contacts extends HubSpot_BaseClient{
    ...
    **/
    public function batch_create_update_contact($DataArray){
        $endpoint = 'contact/batch';
        $Batch = array();
        $count = 0;
        foreach ($DataArray as $person) {
            $Batch[$count]['email'] = $person['email'];

            $properties = array();
            foreach ($person['properties'] as $key => $value) {
                array_push($properties, array("property"=>$key,"value"=>$value));
            }       
            $Batch[$count]['properties'] = $properties;     
            $count = $count +1;
            
        }
        $properties = json_encode($Batch);
        try{
    ...
    * @throws HubSpot_Exception
    **/
    public function create_or_update_contact($params,$User_Email){
        $endpoint = 'contact/createOrUpdate/email/'.$User_Email;
        $properties = array();
